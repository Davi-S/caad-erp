...
│   └── caad_erp.conf.json # Backend runtime configuration file (generated by UI, ignored by git)
├── frontend/           # Frontend Workspace (React 19 + Vite + Mantine + React Query)
│   ├── src/            # Features (POS, Stock, Salesmen, Home, Settings), components, and hooks
│   │   └── config/     # Frontend configuration context, storage helpers, and default values
...
│   ├── USER_GUIDE.md      # Naming conventions, operational best practices, and pitfalls
...
### Configuration and Secrets (caad_erp.conf.json + Frontend Config)

CAAD ERP uses two complementary configuration mechanisms instead of traditional
`.env` files.

**Backend - `caad_erp.conf.json`:** Server-side runtime settings (Mercado Pago
Access Token, payer email, PIX description template) are stored in a JSON file
managed through the application's Settings UI and written by the `settings` tRPC
router.

- **No Developer Setup:** New environments do not require developers to manually copy `.env.example` files or configure bash environment variables.
- **Runtime Modifiability:** Server configuration (like Mercado Pago Access Tokens) can be updated securely at runtime by the user via the frontend UI.
- **Security:** The configuration file is included in `.gitignore` to prevent secret leakage. The backend tRPC router intercepts and hides sensitive tokens (returning a `hasAccessToken` boolean) to ensure secrets are never leaked to the browser during read queries.

**Frontend - `localStorage` via `AppConfigProvider`:** Client-side preferences
(store name, default Excel export filename, and the PIX template) are persisted
in `localStorage` and made available to all components through a React context
(`frontend/src/config/`). This separation keeps purely presentational preferences
out of the backend configuration file.

- **`AppConfigProvider`:** Wraps the application at the root, reads saved config from `localStorage` on mount, and exposes `config` and `setConfig` via `useAppConfig()`.
- **Config Import/Export:** The Settings page can serialize the frontend config to a `.json` file for backup or cross-device sharing, and import it back, restoring all preferences in a single step.
...
modules (`products`, `salesmen`, `transactions`, `reports`, `settings`). It
constructs context containing the active database client and routes validated
inputs into BLL handlers.

The `settings` router is responsible for reading and writing the
`caad_erp.conf.json` runtime configuration file. It intercepts sensitive fields
(such as the Mercado Pago Access Token) on read queries, returning a
`hasAccessToken` boolean instead of the raw secret to prevent leaking credentials
to the browser.
