...
### Configuration and Secrets (caad_erp.conf.json)

Unlike traditional Node.js applications that rely on `.env` files and `process.env`, CAAD ERP uses a dynamic JSON configuration file (`caad_erp.conf.json`) managed directly through the application's UI settings page.
